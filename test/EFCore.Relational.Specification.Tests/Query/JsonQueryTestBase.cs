    ...
    where TFixture : JsonQueryFixtureBase, new()
{
    ...
    public virtual Task Project_json_entity_FirstOrDefault_subquery(bool async)
        => AssertQuery(
            async,
            ss => ss.Set<JsonEntityBasic>().OrderBy(x => x.Id).Select(x => ss.Set<JsonEntityBasic>().OrderBy(xx => xx.Id).Select(xx => xx.OwnedReferenceSharedRoot).FirstOrDefault().OwnedReferenceSharedBranch));
    ...
    public virtual Task Project_json_entity_FirstOrDefault_subquery_deduplication(bool async)
        => AssertQuery(
            async,
            ss => ss.Set<JsonEntityBasic>().OrderBy(x => x.Id).Select(x => ss.Set<JsonEntityBasic>().OrderBy(xx => xx.Id).Select(xx => new { x.OwnedReferenceSharedRoot.OwnedCollectionSharedBranch, xx.OwnedReferenceSharedRoot, xx.OwnedReferenceSharedRoot.OwnedReferenceSharedBranch }).FirstOrDefault()));
