...
public abstract class TableSplittingTestBase : NonSharedModelTestBase
{
    ...
    public virtual async Task BulkDelete_throws_for_table_sharing(bool async)
    {
        await InitializeAsync(OnModelCreating);
            ...
            await TestHelpers.ExecuteWithStrategyInTransactionAsync(
                ...
                async context => Assert.Contains(
                    RelationalStrings.NonQueryTranslationFailedWithDetails(
                        "", RelationalStrings.BulkDeleteOnTableSplitting("BulkDelete", "Vehicles"))[21..],
                    (await Assert.ThrowsAsync<InvalidOperationException>(() => context.Set<Vehicle>().BulkDeleteAsync())).Message));
        }
        else
        {
            TestHelpers.ExecuteWithStrategyInTransaction(
                ...
                context => Assert.Contains(
                    RelationalStrings.NonQueryTranslationFailedWithDetails(
                        "", RelationalStrings.BulkDeleteOnTableSplitting("BulkDelete", "Vehicles"))[21..],
                    Assert.Throws<InvalidOperationException>(() => context.Set<Vehicle>().BulkDelete()).Message));
        }
    }
