...
public abstract class EntitySplittingTestBase : NonSharedModelTestBase
{
    ...
    public virtual async Task BulkDelete_throws_for_entity_splitting(bool async)
    {
        await InitializeAsync(OnModelCreating, sensitiveLogEnabled: true);
            ...
            await TestHelpers.ExecuteWithStrategyInTransactionAsync(
                ...
                async context => Assert.Contains(
                    RelationalStrings.NonQueryTranslationFailedWithDetails(
                        "", RelationalStrings.BulkOperationOnEntitySplitting("BulkDelete", "MeterReading"))[21..],
                    (await Assert.ThrowsAsync<InvalidOperationException>(() => context.MeterReadings.BulkDeleteAsync())).Message));
        }
        else
        {
            TestHelpers.ExecuteWithStrategyInTransaction(
                ...
                context => Assert.Contains(
                    RelationalStrings.NonQueryTranslationFailedWithDetails(
                        "", RelationalStrings.BulkOperationOnEntitySplitting("BulkDelete", "MeterReading"))[21..],
                    Assert.Throws<InvalidOperationException>(() => context.MeterReadings.BulkDelete()).Message));
        }
    }
