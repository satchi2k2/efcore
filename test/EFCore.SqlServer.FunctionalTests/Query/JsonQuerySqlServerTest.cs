...
ORDER BY [j].[Id]");
    ...
    public override async Task Json_entity_with_inheritance_basic_projection(bool async)
    {
        ...
        AssertSql(
            @"SELECT [j].[Id], [j].[Discriminator], [j].[Name], [j].[Fraction], JSON_QUERY([j].[collection_on_base],'$'), JSON_QUERY([j].[reference_on_base],'$'), JSON_QUERY([j].[collection_on_derived],'$'), JSON_QUERY([j].[reference_on_derived],'$')
...
WHERE [j].[Discriminator] = N'JsonEntityInheritanceDerived'");
    ...
    public override async Task Json_entity_with_inheritance_project_navigations(bool async)
    {
        ...
        AssertSql(
            @"SELECT [j].[Id], JSON_QUERY([j].[reference_on_base],'$'), JSON_QUERY([j].[collection_on_base],'$')
FROM [JsonEntityInheritanceBase] AS [j]");
    }
