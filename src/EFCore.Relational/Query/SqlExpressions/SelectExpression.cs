...
public sealed partial class SelectExpression : TableExpressionBase
{
        ...
        : base(null)
    {
        switch (entityType.GetMappingStrategy())
        {
            ...
            default:
            {
                ...
                foreach (var ownedJsonNavigation in GetAllNavigationsInHierarchy(entityType).Where(n => n.ForeignKey.IsOwnership && n.TargetEntityType.IsMappedToJson()))
                {
                    var targetEntityType = ownedJsonNavigation.TargetEntityType;
                    var jsonColumnName = targetEntityType.MappedToJsonColumnName()!;
                    var jsonColumnTypeMapping = (RelationalTypeMapping)targetEntityType[RelationalAnnotationNames.MapToJsonTypeMapping]!;
        ...
        QuerySplittingBehavior querySplittingBehavior)
    {
            ...
            for (var i = 0; i < _clientProjections.Count; i++)
            {
                ...
                switch (value)
                {
                        ...
                        || shapedQueryExpression.ResultCardinality == ResultCardinality.SingleOrDefault:
                    {
                        ...
                        jsonClientProjectionDeduplicationMap = BuildJsonProjectionDeduplicationMap(_clientProjections.OfType<JsonQueryExpression>());
        ...
        static Dictionary<JsonQueryExpression, JsonScalarExpression> BuildJsonProjectionDeduplicationMap(IEnumerable<JsonQueryExpression> projections)
        {
        ...
        ConstantExpression AddJsonProjection(JsonQueryExpression jsonQueryExpression, JsonScalarExpression jsonScalarToAdd)
        {
            ...
            var keyProperties = jsonQueryExpression.EntityType.FindPrimaryKey()!.GetMappedKeyProperties();
...
//                || expression is JsonProjectionExpression
    ...
    private static Expression MakeNullable(Expression expression, bool nullable)
    {
        if (nullable)
        {
            ...
            // TODO: need makenullable for jsonqueryexpression also?
        }
