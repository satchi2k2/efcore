...
public class RelationalQueryableMethodTranslatingExpressionVisitor : QueryableMethodTranslatingExpressionVisitor
{
            ...
            if (source is ShapedQueryExpression shapedQueryExpression)
            {
                ...
                switch (method.Name)
                {
                    case nameof(RelationalQueryableExtensions.BulkDelete)
                        ...
                        return TranslateBulkDelete(shapedQueryExpression)
                            ?? throw new InvalidOperationException(
    ...
    protected virtual NonQueryExpression? TranslateBulkDelete(ShapedQueryExpression source)
    {
        ...
        {
            AddTranslationErrorDetails(RelationalStrings.BulkOperationOnNonEntityType(nameof(RelationalQueryableExtensions.BulkDelete)));
            return null;
        }
        ...
        if (mappingStrategy == RelationalAnnotationNames.TptMappingStrategy)
        {
            AddTranslationErrorDetails(
                RelationalStrings.BulkOperationOnTPT(nameof(RelationalQueryableExtensions.BulkDelete), entityType.DisplayName()));
            return null;
        }
            ...
            && entityType.GetDirectlyDerivedTypes().Any())
        {
            ...
            AddTranslationErrorDetails(
                RelationalStrings.BulkOperationOnTPC(nameof(RelationalQueryableExtensions.BulkDelete), entityType.DisplayName()));
            return null;
        }
        ...
        {
            AddTranslationErrorDetails(
                RelationalStrings.BulkOperationOnEntitySplitting(
                    nameof(RelationalQueryableExtensions.BulkDelete), entityType.DisplayName()));
            return null;
        }

        var selectExpression = (SelectExpression)source.QueryExpression;
        if (IsValidSelectExpressionForBulkDelete(selectExpression, entityShaperExpression, out var tableExpression))
        {
            ...
            {
                AddTranslationErrorDetails(
                    RelationalStrings.BulkDeleteOnTableSplitting(
                        nameof(RelationalQueryableExtensions.BulkDelete), tableExpression.Table.SchemaQualifiedName));
        ...
        {
            AddTranslationErrorDetails(
                RelationalStrings.BulkOperationOnKeylessEntityTypeWithUnsupportedOperator(
