    ...
    public override Expression? Visit(Expression? expression)
    {
            ...
            if (_indexBasedBinding)
            {
                switch (expression)
                {
                    ...
                    case MaterializeCollectionNavigationExpression materializeCollectionNavigationExpression:
                            ...
                            && methodCallSubquery.Method.IsGenericMethod)
                        {
                            ...
                            if (methodCallSubquery.Method.IsGenericMethod
                                ...
                                && methodCallSubquery.Arguments[0] is JsonQueryExpression jsonQueryExpression
                                && jsonQueryExpression.Navigation.IsCollection)
                                //&& methodCallSubquery.Arguments[0] is JsonCollectionResultExpression2 jcre)
                            {
                                return Visit(jsonQueryExpression);
                            }
                    ...
                    case JsonQueryExpression jsonQueryExpression
                        when jsonQueryExpression.Navigation.IsCollection:
                    {
                        _clientProjections!.Add(jsonQueryExpression);

                        return new CollectionResultExpression(
                            new ProjectionBindingExpression(_selectExpression, _clientProjections!.Count - 1, jsonQueryExpression.Navigation.ClrType),
                            jsonQueryExpression.Navigation,
                            jsonQueryExpression.Navigation.ClrType.GetSequenceType());
                    }
        ...
        static Expression RemoveJsonIncludes(Expression expression)
            => expression is IncludeExpression includeExpression
                && includeExpression.Navigation.DeclaringEntityType.IsMappedToJson()
